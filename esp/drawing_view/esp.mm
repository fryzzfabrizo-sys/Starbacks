...
static inline void ESPAddHealthBar(CGMutablePathRef bgPath, CGMutablePathRef fillPath,
    ...
    const CGFloat gap   = 3.0f;
    const CGFloat bdr   = 0.8f;

    CGFloat x = boxRect.origin.x - gap - barW;
    CGFloat y = boxRect.origin.y;
    CGFloat h = boxRect.size.height;

    CGPathAddRect(bgPath, NULL, CGRectMake(x - bdr, y - bdr, barW + bdr * 2.0f, h + bdr * 2.0f));
...
static inline BOOL RenderFOVCirclePath(CGMutablePathRef path, CGFloat vw, CGFloat vh, BOOL aiming, float fov) {
...
static void ESPTextCallback(void *ctx, NSString *str, CGRect frame, UIColor *color, CGFloat fontSize, BOOL leftAligned) {
    ...
    [view addText:str frame:f color:color fontSize:finalSize leftAligned:leftAligned];
}
...
- (CAShapeLayer *)makeShapeLayer:(UIColor *)stroke fill:(UIColor *)fill lineWidth:(CGFloat)lw zPos:(CGFloat)z {
...
- (void)configureRenderingLayers {
    ...
    UIColor *green = [UIColor colorWithRed:0.0f green:0.85f blue:0.2f alpha:1.0f];
    UIColor *red = [UIColor colorWithRed:1.0f green:0.0f blue:0.0f alpha:1.0f];
    UIColor *boneWhite = [UIColor whiteColor];

    self.lineKnockLayer  = [self makeShapeLayer:red fill:nil lineWidth:0.7f zPos:1];
    self.lineBotLayer    = [self makeShapeLayer:white fill:nil lineWidth:0.7f zPos:1];
    self.linePlayerLayer = [self makeShapeLayer:white fill:nil lineWidth:0.7f zPos:1];

    self.boxBotLayer     = [self makeShapeLayer:green fill:nil lineWidth:0.7f zPos:3];
    self.boxPlayerLayer  = [self makeShapeLayer:green fill:nil lineWidth:0.7f zPos:3];
    self.boxKnockLayer   = [self makeShapeLayer:red fill:nil lineWidth:0.7f zPos:1];

    self.fovLayer        = [self makeShapeLayer:green fill:nil lineWidth:0.7f zPos:0];
...
- (void)updateFrame {
    ...
    @autoreleasepool {
        ...
        ApplyPath(self.boxKnockLayer,     buffers.boxKnockPath,     buffers.boxKnockDirty);
        ApplyPath(self.lineKnockLayer,    buffers.lineKnockPath,    buffers.lineKnockDirty);
        ApplyPath(self.boxBotLayer,       buffers.boxBotPath,       buffers.boxBotDirty);
        ApplyPath(self.boxPlayerLayer,    buffers.boxPlayerPath,    buffers.boxPlayerDirty);
        ApplyPath(self.lineBotLayer,      buffers.lineBotPath,      buffers.lineBotDirty);
        ApplyPath(self.linePlayerLayer,   buffers.linePlayerPath,   buffers.linePlayerDirty);
        ...
        if (stats.inMatch) {
            ...
            if (isCount) {
                ...
                if (totalEnemies == 0) {
                    ...
                    self.statusLayer.string = [NSString stringWithFormat:@"Player : %d", totalEnemies];
                    self.statusLayer.foregroundColor = [UIColor whiteColor].CGColor;
                    self.statusLayer.fontSize = 20.0f;
                    self.statusLayer.font = (__bridge CFTypeRef)[UIFont systemFontOfSize:20.0f weight:UIFontWeightBold];
                    self.statusLayer.frame = CGRectMake(vw / 2.0f - 120.0f, 60.0f, 240.0f, 30.0f);
                    self.statusLayer.hidden = NO;
                }
            } else {
